...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	//rV средняя скорость хотьбы в км/ч
	rV := meanSpeed(action, duration)
	return (runningCaloriesMeanSpeedMultiplier * rV * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	//wInMsec средняя скорость бега в м/с
	wVInMsec := meanSpeed(action, duration) * kmhInMsec
	return (walkingCaloriesWeightMultiplier*weight + (math.Pow(wVInMsec, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	//rV средняя скорость при плавании в км/ч
	sV := swimmingMeanSpeed(lengthPool, countPool, duration)
	return (sV + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
}
